...
- **Firestore rules + indexes deploy from CI too** (added 2026-08-13), in a step that runs *before* hosting. `FirebaseExtended/action-hosting-deploy` is hosting-only and never reads `firebase.json`'s `firestore` block, so rules changes previously shipped nowhere — CI went green, the site updated, and the app hit the old rules with permission-denied errors that read like application bugs. Rules first because rule changes are additive: old code under new rules is harmless, new code under old rules breaks. The step authenticates the `firebase-tools` CLI with the same service-account secret via `GOOGLE_APPLICATION_CREDENTIALS` and omits `--force`, so index deletions are reported rather than silently applied
