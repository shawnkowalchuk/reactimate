...
- **Testing:** vitest, **146 tests passing across 15 files**. Run `npm test -- --run`. Don't merge a regression.
- **Responsive scope:** the marketing home page and `/admin/*` are mobile-ready and must stay that way; the editor (`/app`, `src/components/layout/*`) is deliberately desktop-only. Mobile work is additive — add base classes plus `sm:`/`md:` overrides, never weaken an existing `lg:` class desktop depends on. Verify at 390px with `document.documentElement.clientWidth`, NOT `window.innerWidth`, which misreports in the preview pane and will tell you a broken page is fine.
- **Firestore rules deploy from CI.** The `deploy` job ships `firestore.rules` + indexes *before* hosting, so new code never runs against stale rules. Manual fallback: `firebase deploy --only firestore:rules`. (The `FirebaseExtended/action-hosting-deploy` action is hosting-only and ignores `firebase.json`'s `firestore` block — hence the separate step.)
